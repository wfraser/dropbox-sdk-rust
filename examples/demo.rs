...
async fn list_directory<'a, F>(client: &'a dyn HttpClient<F>, path: &str, recursive: bool)
    -> dropbox_sdk::Result<DirectoryIterator<'a, F>, files::ListFolderError>
...
impl<'a, F: Future<Output=HttpResult>> DirectoryIterator<'a, F> {
    pub async fn next(&mut self) -> Option<dropbox_sdk::Result<files::Metadata, files::ListFolderContinueError>> {
